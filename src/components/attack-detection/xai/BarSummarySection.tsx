...
import { fetchAttackBarSummaryXAIExplanation } from "@/utils/client/fetchAttackSummaryXAI";
...
type BarSummarySectionProps = {
  attackType: string;
  barSummaryData: any; // Replace with actual type if available
  isLoading: boolean;
};

export function BarSummarySection({
  attackType,
  barSummaryData,
  isLoading,
}: BarSummarySectionProps) {
  ...
  const { data: explanationData, isLoading: explanationLoading } = useSWR(
    ...
    () => fetchAttackBarSummaryXAIExplanation({ attack_type: attackType }),
    {
      ...
      onError: async (error) => {
        await Swal.fire({
          icon: "error",
          title: "Failed to fetch bar chart explanation",
        ...
        console.error("Failed to fetch bar summary explanation:", error);
      },
    }
  );
  ...
  return (
    <div className="bg-white rounded-lg border-2 shadow-sm flex flex-col">
      <SummaryBarChart attackType={attackType} data={barSummaryData} />
      <div className="p-4 border-t border-gray-200">
        ...
        <div className="mt-4">
          <div className="mb-2 text-lg font-semibold text-orange-500">
            Need help understanding the bar chart?
          </div>
          {!explanationRequested && (
            <Button
              ...
              disabled={explanationLoading || !barSummaryData}
              id="explanation-button-gemini-bar"
            >
              Ask Gemini?
              <img
                src="/gemini-icon.svg"
